...
export class GlobalEvents {
  ...
  constructor() {
  window.addEventListener('mousemove', this.types_.onMouseMove.trigger);
  window.addEventListener('mouseup', this.types_.onMouseUp.trigger);
  }
...
export namespace GlobalEvents {
  ...
  export class EventType<T> {
    ...
    trigger = (event: T) => {
      ...
      if ((event as unknown as Event).preventDefault && preventDefault) {
        (event as unknown as Event).preventDefault();  // <-- add this line
      }
