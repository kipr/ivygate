...
import { Vector2, clamp } from '../../util/math';
...
import { GlobalEvents, GLOBAL_EVENTS } from '../../util/GlobalEvents';
...
class ScrollArea extends React.PureComponent<Props, State> {
  ...
  private applyScrolling = (newOffset: Vector2, isUsingBar: boolean, invert: boolean) => {
    ...
    if (maxTop > 0) {
      ...
      let topDiff = diff.y;
      if (isUsingBar) {
        const scrollBarHeight = this.vScrollHeight;
        const scrollTrackHeight = outerSize.y - scrollBarHeight;
        const scrollFactor = scrollTrackHeight > 0 ? (this.maxTop / scrollTrackHeight) : 1;
        topDiff *= scrollFactor;
      }
