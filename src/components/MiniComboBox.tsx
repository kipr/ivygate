...
class MiniComboBox extends React.PureComponent<MiniComboBox.Props, MiniComboBox.State> {
  constructor(props: MiniComboBox.Props) {
    super(props);
    this.state = {
      focus: false
    };
  ...
  private onMiniComboBoxRootClick_ = (event: MouseEvent) => {
    this.setState({
      focus: false
    }, () => {
      if (this.onFocusChange_) {
  ...
  render() {
    ...
    if (this.ref_ && focus) {
      ...
      COMBO_BOX_ROOT.onclick = this.onMiniComboBoxRootClick_;
    } else if (COMBO_BOX_ROOT.onclick === this.onMiniComboBoxRootClick_) {
    ...
    return (
      <Container ref={this.bindRef_} style={style} className={className} theme={theme} onClick={this.onClick_} $focus={focus} $minimal={minimal}>
        <CurrentOptionContainer theme={theme}><Text style={{ fontSize: '10px', lineHeight: '2' }} text={options[index].text} /></CurrentOptionContainer>
        ...
        {ReactDom.createPortal((focus && this.ref_)
          ? <DropDown theme={theme} style={dropDownStyle}>
            {options.map((option, i) => (
              ...
              >
                <Text style={{ fontSize: '10px', lineHeight: '2' }} text={option.text} />
              </OptionContainer>
            ))}
          </DropDown>
          : null
...
namespace MiniComboBox {
  ...
  export interface Props extends StyleProps, ThemeProps {
    ...
    innerRef?: (ref: MiniComboBox) => void;
